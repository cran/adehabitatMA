...
these functions in the package \texttt{adehabitatMA}. I demonstate the use
...
\subsection{Controlling the options}

The user familiar with the use of the package \texttt{sp} might have
noted, in the previous section, that the content of the object of
class \texttt{SpatialPixelsDataFrame} was printed in an unusual way.
Actually, the package \texttt{adehabitatMA} defines its own printing
methods.  The use of these printing methods can be controlled with
the function \texttt{adeoptions}.  The default value for the
\texttt{shortprint} option is \texttt{TRUE}:

<<>>=
adeoptions(shortprint=TRUE)
@

which causes the \texttt{Spatial} objects to be printed in that way;

<<>>=
lynxjura$map
@

The user might find disturbing the use of these methods, and
prefer the original printing methods proposed by the package
\texttt{sp}. In this case, the \texttt{shortprint} option should be
turned to \texttt{FALSE}.
